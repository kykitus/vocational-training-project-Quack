...
func _physics_process(delta: float) -> void:
	...
	#$AppHolder.position.x = clampf($AppHolder.position.x, -1 * $AppHolder.size.x + 615, 200)
	if not is_Dragging:
		$AppHolder.position.x += velocity
		if $AppHolder.position.x > 200:
			$AppHolder.position.x = lerp($AppHolder.position.x, 200.0, 0.15)
		elif $AppHolder.position.x < -1 * $AppHolder.size.x + 1075:
			$AppHolder.position.x = lerp($AppHolder.position.x, -1 * $AppHolder.size.x + 1075, 0.15)
